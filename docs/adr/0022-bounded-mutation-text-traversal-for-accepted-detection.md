...
Attribute 기반 presentation 감지가 필요한 Coding Platform은 adapter가 발견해 등록한 presentation root만 별도 관찰한다. 관찰 attribute는 visibility 판정에 필요한 `aria-hidden`, `hidden`, `class`, `style`로 제한하며, page 전체 attribute mutation을 수집하지 않는다. Attribute callback은 등록된 root의 이전/현재 presentation state를 비교할 뿐 `mutation.target`이나 document의 큰 subtree text를 다시 scan하지 않는다.
