...
- 대상 저장소: 사용자가 GitHub fine-grained PAT로 접근 가능한 repository 중 선택한다. 특정 repository를 제품 기본값으로 고정하지 않는다.
...
- Options는 입력된 PAT로 접근 가능한 repository 목록을 보여주고 사용자가 대상 repository를 선택하게 한다.
...
- 테스트가 실패하면 Options는 auth failed, token expired, no accessible repositories, repository not found, branch not found, branch create failed, rate limited, network failed 중 가장 가까운 복구 가능한 상태를 보여준다.
