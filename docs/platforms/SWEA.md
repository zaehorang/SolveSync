...
이 문서의 DOM 사실은 2026-08-14 로그인 상태의 실제 Chrome 관찰에서 출발했고, **2026-08-18 문제 1206을 실제로 Accepted 제출해 감지부터 GitHub commit까지 전 구간을 실증했다.** 판정 결과는 [확인된 전제](#확인된-전제)에 있다.
...
실패 신호는 두 종류이고 성격이 다르다.

| 실패 | 실측 여부 | text |
| --- | --- | --- |
| 컴파일 오류 | 2026-08-18 실측 | `제출 오류컴파일 오류 : 오류 메세지 : ...확인닫기` |
| 채점 실패 | page script 독해만. 실제 제출로 보지 못함 | `채점용 input 파일로 채점한 결과 fail 입니다.` |

둘 다 Accepted 접두사와 겹치지 않는다. 컴파일 오류 layer가 떴을 때 toast와 event가 생기지 않는 것은 실측으로 확인했다.
...
| 5 | 실패 제출 문구가 Accepted 신호와 겹치지 않는다 | 컴파일 오류는 확인. 채점 실패 문구는 아직 실제 제출로 보지 못했다 |
...
전제 5의 채점 실패 경로는 남은 확인 대상이다. 잘못된 sync가 생길 수 있는 유일한 방향이므로 다음 수동 검증에서 실제 오답을 제출해 확인한다.
