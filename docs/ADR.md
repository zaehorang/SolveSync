...
> **Description**: 주요 설계 결정, 결정 이유, 트레이드오프를 기록한 문서다.
...
### ADR-014: 대상 저장소는 플랫폼 기준 폴더를 우선한다
...
### ADR-016: processed marking은 commit 성공 후에만 한다
...
### ADR-017: Storage schema는 version을 포함한다
...
### ADR-018: Runtime message는 typed union으로 관리한다
...
### ADR-019: 최소 host permission만 요청한다
...
### ADR-020: 대상 repository와 branch는 사용자가 선택한다
...
### ADR-021: Branch 생성은 명시적 사용자 action으로만 수행한다
...
### ADR-022: Accepted 감지는 mutation 범위의 bounded text traversal을 사용한다
...
### ADR-023: Content script는 별도 IIFE bundle로 빌드한다
...
### ADR-025: Harness validation과 dirty recovery policy를 분리한다
