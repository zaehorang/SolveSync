...
v4 migration은 legacy storage와 runtime payload를 backward-compatible parser로 읽되, 새 write path는 v4/domain name만 쓴다. Runtime message alias는 ingress compatibility 전용이며, 새 UI/background code는 v4 message type과 field만 emit한다. ADR source of truth는 `docs/adr/` 아래 파일이며 `docs/ADR.md`는 사용하지 않는다.
