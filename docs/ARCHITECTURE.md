...
├── popup/           # Auto Sync 토글, Sync History, failure, retry UI
└── shared/          # 타입, Coding Platform policy, mapping, runtime message, storage schema, 순수 로직
```
...
- Programmers는 Accepted 감지 직후 현재 editor code, language, title을 Accepted Editor Snapshot으로 추출한다.
...
- Coding Platform policy로 root folder, README path, index path, marker, commit message prefix를 제공한다.
...
- settings, in-flight lock, processed submissions, Sync History, Retry Bundle은 `chrome.storage.local`에 저장하고 재시작 후 복구 가능해야 한다.
- service worker wake-up 후 storage를 다시 읽어 현재 요청을 판단한다.
- 중복 방지는 memory cache가 아니라 storage에 저장된 processed key와 in-flight key를 기준으로 한다.
...
→ background가 Coding Platform source resolver로 problem/source/key 확정
→ background가 key lock 획득
→ background가 solution path, Solution Catalog 갱신, README 갱신 생성
→ background가 GitHub Git Data API로 commit 생성
→ background가 processed key와 Sync History 저장
...
- Coding Platform policy는 root path, README path, index path, marker, initial README title, commit message prefix를 제공한다.
...
- LeetCode Sync Deduplication Key는 `platform`, `submissionId`, `titleSlug`, language 조합이다.
...
- language와 code는 현재 문제 페이지의 Accepted Editor Snapshot에서 추출한다.
...
- Programmers Sync Deduplication Key는 실제 submission id가 없으면 `programmers:{lessonId}:{language}:{codeHash}` 형식의 deterministic key를 사용한다.
...
- commit 흐름은 다음 순서를 따른다.
  ...
  - 새 commit 생성
  - branch ref update
- branch가 이동해 ref update가 실패하면 최신 branch 상태를 다시 읽고 README와 Solution Catalog를 재생성한 뒤 한 번만 재시도한다.
...
## README와 Solution Catalog
각 solution catalog가 Sync Repository 안에서 README와 풀이 진행표를 재생성하기 위한 source of truth다. 중복 처리, Sync History, Retry 상태의 source of truth는 각각 storage의 processed submissions, sync history, Retry Bundle이다.
...
LeetCode-only storage에서 Coding Platform-aware storage로 바뀔 때는 schema version을 올리고 기존 deduplication key와 Sync History 항목에는 `platform: "leetcode"`를 채운다. Migration은 기존 state를 임의 삭제하지 않고, malformed state만 해당 key의 empty fallback으로 복구한다.

Keys:
- `settings`: version, PAT, selected repository owner/name, selected branch, Auto Sync, connection status.
- `processedSubmissions`: version, 처리된 Sync Deduplication Key 목록.
- `syncHistory`: version, Sync History의 최근 20개 항목.
- `retryPayloads`: version, GitHub commit retry가 가능한 Retry Bundle 목록.
- `inFlightSyncs`: version, 현재 처리 중인 Sync Deduplication Key lock 목록. 각 lock은 생성 시각을 저장하고 10분 TTL을 가진다.

## 동시성과 Retry Lifecycle
- Sync Deduplication Key는 `platform`, `submissionId`, `titleSlug`, language 조합이다.
- 새 sync 시작 전 10분이 지난 stale in-flight lock을 정리한다.
- background는 sync 시작 전에 `inFlightSyncs`에 key lock을 기록한다.
- 같은 key가 이미 in-flight이면 새 요청은 중복으로 처리하지 않고 현재 상태를 반환한다.
- GitHub commit 성공 후에만 `processedSubmissions`에 key를 기록한다.
