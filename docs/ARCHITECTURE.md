...
표준 코드/domain 용어는 `CONTEXT.md`를 따른다. TypeScript identifier, runtime message payload, `chrome.storage.local` schema는 v4부터 다음 이름을 새 계약으로 사용한다.

| Legacy name | v4/domain name |
| --- | --- |
...
v4 migration은 legacy storage와 runtime payload를 backward-compatible parser로 읽되, 새 write path는 v4/domain name만 쓴다. ADR source of truth는 `docs/adr/` 아래 파일이며 `docs/ADR.md`는 사용하지 않는다.
...
→ background가 solution path, Solution Catalog 갱신, Solution README 갱신 생성
...
- language와 code는 현재 문제 페이지의 Accepted Editor Snapshot에서 추출하고 `ProgrammersAcceptedEditorSnapshot`으로 표현한다.
...
Solution Catalog는 v2 schema를 사용한다. 실제 파일 경로는 호환성을 위해 유지한다.
...
- `inFlightSyncDeduplicationKeys`: version, 현재 처리 중인 Sync Deduplication Key lock 목록. 각 lock은 생성 시각을 저장하고 10분 TTL을 가진다.
