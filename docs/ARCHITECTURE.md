...
│   └── client/      # LeetCode GraphQL, GitHub Git Data API 실행 코드
├── content/         # LeetCode 페이지 관찰과 toast UI
├── options/         # 설정, PAT 안내, connection test UI
...
### Options Page
- GitHub PAT, owner, repo, branch, Auto Sync 설정을 저장한다.
- fine-grained PAT 설정 방법을 안내한다.
- GitHub repository와 branch API를 대상으로 connection test를 실행한다.
...
## 대상 저장소 경로
대상 저장소는 설정 가능하다. 기본 대상은 `zaehorang/Swift_Algorithm`이다.
...
생성된 Swift 파일은 `swift/SwiftAlgorithm` 아래에 저장하지 않는다. 기본 대상 저장소에서 해당 폴더는 Xcode build source 전용이다.
...
Keys:
- `settings`: version, PAT, owner, repo, branch, Auto Sync.
...
- popup/options to background: settings read/write, connection test, retry.
