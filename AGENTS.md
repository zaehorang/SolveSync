...
- Programmers 자동 동기화는 v2 후보이며 v1 범위가 아니다.
...
- content detector는 LeetCode DOM class selector나 페이지 전체 텍스트 scan에 의존하지 않는다. `MutationObserver`가 전달한 변경 범위 안에서 제한된 leaf text 후보를 검사해 Accepted 이벤트만 감지한다.
...
- 대상 저장소에 풀이 폴더, `leetcode/README.md`, `leetcode/.leetcode-sync/index.json`이 없어도 첫 GitHub commit에서 생성해야 한다.
...
## README와 Index 규칙
- 대상 저장소의 `leetcode/.leetcode-sync/index.json`이 LeetCode 동기화 메타데이터의 source of truth다.
- v1은 README를 항상 갱신한다. README 갱신 토글이나 README 비갱신 모드는 만들지 않는다.
- `leetcode/README.md`는 index를 기준으로 생성한다.
