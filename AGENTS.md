...
- 설계 결정과 tradeoff는 `docs/ADR.md`를 따른다.
...
## Project Map
- `src/content`: 문제 페이지 관찰, Accepted 감지, Programmers editor snapshot, toast, background messaging.
- `src/background`: sync orchestration, source resolver, storage, retry, history.
- `src/background/client`: LeetCode와 GitHub API client. API 변경 영향은 여기서 막는다.
- `src/options`: PAT, repository/branch 선택, branch 생성, Auto Sync, connection test UI.
- `src/popup`: Auto Sync toggle, 최근 history, 실패 상세, retry UI.
- `src/shared`: 타입, platform policy, message union, language/path mapping, README/index, storage schema, error normalization.
...
## High-Risk Rules
- `processedSubmissions`는 GitHub commit 성공 후에만 기록한다.
- 같은 sync identity는 storage 기반 in-flight lock으로 중복 처리를 막는다.
- retry payload에는 solution code가 임시 저장될 수 있으므로 UI disclosure와 TTL/cap 정책을 유지한다.
- Programmers는 공식 제출 상세 API를 전제로 하지 않고 Accepted 직후 editor snapshot을 source로 쓴다.
- README는 platform index의 projection이다. managed marker 밖 사용자의 수동 내용은 보존한다.
...
- architecture, storage, runtime message, API boundary 변경: `docs/ARCHITECTURE.md`와 `docs/ADR.md` 확인.
