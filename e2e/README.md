...
편의의 문제가 아니라 사이트마다 사정이 다르다. 세 줄로 정리하면 이렇다.

| | 세션이 사는 곳 | 자동 로그인 | 이유 |
| --- | --- | --- | --- |
| LeetCode | Verification Profile의 쿠키 | **하지 않는다** | Cloudflare Turnstile이 붙어 있어 자동 입력 자체가 bot 판정 위험이다. 한 번 걸리면 그 계정으로 도는 캡처·Contract Check·풀사이클이 전부 막힌다 |
| Programmers | Verification Profile의 쿠키 | **하지 않는다** | 검증 계정이 Google 로그인이다. 자동화하려면 Google의 인증 흐름을 타야 하고 거기서 2단계 인증과 bot 판정에 걸린다 |
| SW Expert Academy | 브라우저 프로세스 안에만 | **한다** | `SESSION` 쿠키가 만료 기한 없이 발급되는 진짜 session cookie라 브라우저가 꺼지면 사라진다. 실행마다 로그인이 필요해 선택지가 없었다 |
